...
const VariantSchema = new Schema<ProductVariant>(
    {
        ...
        ean: {
            type: String,
            required: true,
            index: { unique: true, sparse: true } // Garantizar unicidad del EAN, sparse para evitar problemas con nulos
        },
...
ProductSchema.pre('save', async function (next) {
    if (this.isModified('name') || !this.slug) {
        ...
        if (baseName) {
            let generatedSlug = slugify(baseName);

            // Verificar si el slug ya existe (solo si es nuevo o cambió el nombre)
            if (this.isNew || this.isModified('name')) {
                const ProductModel = this.constructor as mongoose.Model<ProductDocument>;
                const existing = await ProductModel.findOne({ slug: generatedSlug, _id: { $ne: this._id } });

                if (existing) {
                    // Si el slug existe, agregamos un sufijo corto basado en el ID o aleatorio
                    const suffix = this._id ? this._id.toString().slice(-4) : Math.random().toString(36).substring(2, 6);
