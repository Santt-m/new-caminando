...
export class CarrefourHomeScraper extends BaseScraper {
    ...
    async process(): Promise<void> {
        ...
        await this.page.goto('https://www.carrefour.com.ar/', { waitUntil: 'domcontentloaded', timeout: 60000 });

        logger.info(`[${this.name}] Fetching full category tree via API...`, { module: 'SCRAPER_NODE' });

        // Fetch Category Tree using Node context
        const nodes = await this.page.context().request.get('https://www.carrefour.com.ar/api/catalog_system/pub/category/tree/3')
            .then(res => res.json())
            .catch(err => {
        ...
        for (const node of nodes) {
            try {
                ...
                const categoryDoc = await Category.findOneAndUpdate(
                    {
                        $or: [
                            { 'storeMappings.carrefour.externalId': node.id },
                            { name: node.name, 'storeMappings.carrefour': { $exists: true }, parentCategory: parentId },
                            { slug: generatedSlug }
                        ]
                    },
                    {
