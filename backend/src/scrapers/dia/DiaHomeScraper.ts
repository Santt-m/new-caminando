...
export class DiaHomeScraper extends BaseScraper {
    name = 'DIA_HOME';
    ...
    async process(): Promise<void> {
        ...
        await this.page.goto('https://diaonline.supermercadosdia.com.ar/', { waitUntil: 'domcontentloaded', timeout: 60000 });

        logger.info(`[${this.name}] Fetching full category tree via API...`, { module: 'SCRAPER_NODE' });

        // Fetch Category Tree using Node context
        const nodes = await this.page.context().request.get('https://diaonline.supermercadosdia.com.ar/api/catalog_system/pub/category/tree/3')
            .then(res => res.json())
            .catch(err => {
        ...
        const queue = QueueFactory.getQueue(getQueueName(StoreName.DIA));
        ...
        for (const node of nodes) {
            try {
                ...
                const categoryDoc = await Category.findOneAndUpdate(
                    {
                        $or: [
                            { 'storeMappings.dia.externalId': node.id },
                            { name: node.name, 'storeMappings.dia': { $exists: true }, parentCategory: parentId },
                            { slug: generatedSlug }
                        ]
                    },
                    {
