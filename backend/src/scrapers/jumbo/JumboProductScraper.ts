...
export class JumboProductScraper extends BaseScraper {
    name = 'JUMBO_PRODUCT';
    ...
    async process(data: any): Promise<void> {
        ...
        await this.page.goto('https://www.jumbo.com.ar/', { waitUntil: 'domcontentloaded' });
            ...
            for (const p of products) {
                try {
                    ...
                    for (const item of items) {
                        ...
                        const ean = item.ean || `${StoreName.JUMBO}-${item.itemId}`;
                    ...
                    const mainEan = mainItem.ean || `${StoreName.JUMBO}-${mainItem.itemId}`;
                    ...
                    const sourceIndex = productDoc.sources.findIndex((s: any) => s.store === StoreName.JUMBO && s.storeProductId === productId);
                    const sourceData = {
