...
import { ScraperLogger } from '../shared/utils/ScraperLogger.js';
 ...
 */
export const processScraperJobEnhanced = async (job: Job) => {
    ...
    const scraperLogger = new ScraperLogger({
    ...
    logger.info(`[Enhanced] Ejecutando ${job.name} para ${store}...`, {
    ...
    try {
        ...
        switch (job.name) {
            ...
            case 'EXTRACT_BRANDS':
                result = await extractBrandsFromCategoryEnhanced(page, store, categoryUrl, brandExtractor, scraperLogger);
                break;
        ...
        logger.info(`[Enhanced] Completado ${job.name} para ${store}`, {
            module: 'SCRAPER_ENHANCED',
            categoriesFound: result.categories.length,
            brandsFound: result.brands.length,
            errors: result.errors.length
        ...
        logger.error(`[Enhanced] Error en ${job.name} para ${store}:`, {
            module: 'SCRAPER_ENHANCED',
            error: errorMessage,
...
async function discoverCategories(page: Page, store: StoreName): Promise<ScrapingResult> {
    ...
    try {
        ...
        logger.info(`[Enhanced] Categorías descubiertas para ${store}: ${result.categories.length}`, {
            module: 'SCRAPER_ENHANCED'
        });
        ...
        logger.error(`[Enhanced] Error descubriendo categorías para ${store}:`, {
            module: 'SCRAPER_ENHANCED',
            error: errorMessage
        });
    }
...
async function saveDiscoveryResults(
    ...
    try {
        ...
        logger.info(`[Enhanced] Resultados guardados: ${savedCategories.length} categorías, ${savedBrands.length} marcas`, {
            module: 'SCRAPER_ENHANCED'
        });

        if (errors.length > 0) {
            logger.warn(`[Enhanced] Errores durante el guardado: ${errors.length}`, {
                module: 'SCRAPER_ENHANCED',
        ...
        logger.error(`[Enhanced] Error guardando resultados:`, {
            module: 'SCRAPER_ENHANCED',
            error: errorMsg
        });
